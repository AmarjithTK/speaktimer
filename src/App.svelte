<script>
  ...
  const localstorageset = () => {
    ...
    localStorage.setItem("schoosen",Schoosen.toString());
    localStorage.setItem("vchoosen",Vchoosen.toString());
  ...
  let Schoosen =  localstorageget('schoosen') != 'false' ? localstorageget("schoosen") : soundlist[0]["link"];
  let volumelists = [
  ...
  let Vchoosen = localstorageget('vchoosen') !='false'  ? parseFloat(localstorageget("vchoosen")) : volumelists[1]["volume"];
  // Vchoosen = parseFloat(Vchoosen);
  ...
  const timer = () => {
    ...
    if (secs == 30 && seconds != 0 && mins %2 ==0) {
      ...
      speechSynthesis.speak(
        new SpeechSynthesisUtterance(`${hh} ${mm} ${amPm}`)
      );
    }
    if (seconds == 0) {
      reset();
...
<div class="container">
  <div class="flex-item flex-item1">
    ...
    <p>Choose the soundtrack</p>
    <select name="Sound" id="cars" bind:value={Schoosen}>
      {#each soundlist as sound}
        <option value={sound["link"]}>{sound["title"]}</option>
      {/each}
    </select>
    <br>
    <p>Choose the volume level</p>

    <select name="volume" id="volume" bind:value={Vchoosen}>
...
<audio src={Schoosen} loop bind:this={audio} />
